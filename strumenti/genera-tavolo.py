...
BATTLE_CSS = r'''
    ...
    /* IL LEGNO VERO, NON PIÙ SIMULATO. Fin qui il tavolo di legno era
       tutto gradienti CSS — venatura, nodi, giunte fra le assi, disegnati
       a mano. Buono, ma restava un'approssimazione. Sostituito con una
       texture vera (illustrata apposta, stessa palette di prima: legno
       di noce scuro, luce calda dall'alto, accenti oro/ottone appena
       percepibili sui bordi — senza nessun intarsio o medaglione
       circolare al centro, evitato di proposito). */
    body {
      background:
        url('immagini/decorazioni/tavolo-legno.webp') center / cover no-repeat,
        #1c1108 !important;
    }
    ...
    /* Trasparente apposta: e' un figlio nel flusso normale del body, non
       un elemento a parte — lasciandolo senza sfondo suo, il legno del
       body continua sotto di lui senza cuciture, la stessa immagine
       vista da un'altra finestra invece di due texture diverse
       affiancate. Resta solo un'ombra interna, per un campo di gioco
       leggermente incassato rispetto al resto del tavolo. */
    .table-battlefield {
      position: relative;
      background: transparent;
      box-shadow: inset 0 0 120px rgba(0,0,0,0.5), inset 0 0 40px rgba(0,0,0,0.22);
    }
    /* l'anello inciso al centro, dove si gioca — fermo, come un vero
       intaglio nel legno. Uno solo (prima erano due): con la texture
       vera sotto, un secondo anello era solo rumore in più. */
    .table-battlefield::before {
      content: ''; position: absolute; left: 50%; top: 50%; transform: translate(-50%,-50%);
      width: min(40vh, 300px); height: min(40vh, 300px); border-radius: 50%;
    ...
    /* Erano una fascia verde quasi piena (0.92 di opacita') sopra le
       ...
       Richiesto trasparenza TOTALE: tolto anche il blur, il tavolo
       sotto si vede esattamente com'e', senza nessun velo. */
    ...
    /* IL MONTE DEI SEI MINUTI NON AVVISAVA MAI. Il minuto del turno
       (sopra) diventa rosso e pulsa sotto i 15 secondi, ma il monte
       intero — quello che conta davvero, perché azzerarlo fa perdere la
       partita per attrito e ora costa pure le carte rimaste in mano —
       restava un numero muto fino a zero. Stessa idea, soglia più larga
       (30s su 360 totali: la stessa proporzione, spalmata su un
       cronometro quindici volte più lungo). */
    .match-timer.agli-sgoccioli { color: #ff9db0; animation: battito 1s ease-in-out infinite; }

    /* IL CAMBIO TURNO CAMBIAVA SCRITTA DI SCATTO, SENZA DIRLO.
       "Tocca a te" e "Avversario" si scambiavano di colpo — l'unico modo
       per accorgersi che era cambiato davvero il turno (e non solo che
       si era ridisegnato lo schermo per un altro motivo) era leggere la
       parola. Un lampo d'oro quando il testo cambia per davvero rende
       visibile un cambio che prima si notava solo leggendo. */
    .turn-indicator.cambiato { animation: turnoCambia 0.5s ease-out; }
    @keyframes turnoCambia {
      0% { transform: scale(1.32); color: var(--oro); text-shadow: 0 0 14px rgba(232,196,106,0.75); }
      100% { transform: scale(1); }
    }

    /* VITTORIA E SCONFITTA AVEVANO IL SUONO GIUSTO E NESSUN SEGNO A
       SCHERMO — mostraFine() cambiava solo un titolo di testo, uguale
       che si fosse vinto, perso o pareggiato. I coriandoli sono gli
       stessi di spacchetta.html (stessa tecnica: un div per coriandolo,
       una caduta lineare, si toglie da solo). La sconfitta non ha un suo
       equivalente altrove nel gioco: un lampo rosso ai bordi, breve,
       che si spegne da solo — non deve sembrare un errore del gioco,
       solo il peso della sconfitta. */
    .coriandolo {
      position: fixed; z-index: 950; pointer-events: none; width: 0.9vh; height: 1.6vh; border-radius: 1px;
    ...
    .lampoSconfitta {
      ...
      animation: sconfittaFlash 1.1s ease-out forwards;
    }
    @keyframes sconfittaFlash {
      0%   { box-shadow: inset 0 0 0 0 rgba(200,20,40,0); }
      15%  { box-shadow: inset 0 0 18vh 4vh rgba(200,20,40,0.38); }
      100% { box-shadow: inset 0 0 18vh 4vh rgba(200,20,40,0); }
    }
    ...
    /* Quando i giochi sono tanti si accavallano (vedi stringiGiochi nello
       script): quello che sto puntando deve venire davanti a tutti, non
       restare mezzo nascosto sotto il vicino di destra proprio mentre
       decido se agganciarci una carta. */
    .meld-side .card-column:hover { z-index: 60; }
    ...
    /* ============================================================
       IL JOLLY
       Un primo giro (solo stella + scritta "JOLLY" di lato) si leggeva
       male — segnalato dal vivo — e uno ancora prima (un'illustrazione
       vera) era stato tolto apposta perche' rischiava di sparire nel
       ventaglio. Questa volta l'illustrazione e' un vero jolly da mazzo
       francese (il giullare classico, gia' con "JOLLY" scritto in
       verticale dentro l'immagine stessa): il bordo dorato resta,
       identico a prima, proprio per continuare a garantire il
       riconoscimento a colpo d'occhio anche a carta quasi coperta — cambia
       solo cosa c'e' dentro il bordo. */
    .card.jolly {
      background: url('immagini/decorazioni/jolly.webp') center / cover no-repeat, #fdfcf7 !important;
    ...
    /* NOTA E BOTTONI A FIANCO, NON SOPRA — segnalato da chi apriva la
       carta sul telefono: prima poggiavano su un velo scuro sul bordo
       inferiore della carta, e coprivano comunque un pezzo
       dell'illustrazione. Il tavolo e' bloccato in orizzontale (vedi
       "l'orizzontale forzato" piu' sopra nel foglio), quindi la
       larghezza avanza: .colonna e' una riga, la carta da un lato, i
       controlli dall'altro, e la carta puo' restare intera. */
    #veloCarta .colonna {
      display: flex; flex-direction: row; align-items: center; justify-content: center;
      gap: 22px; max-height: 100%; max-width: 100%;
    }
    #veloCarta .carta-illustrata {
      height: min(94vh, 720px); width: auto; aspect-ratio: 0.71;
      flex: 0 0 auto;
      border-radius: 12px;
    ...
    #veloCarta .controlli {
      ...
      width: min(30vw, 200px); flex: 0 0 auto;
    }
    #veloCarta .bottoni { display: flex; flex-direction: column; gap: 10px; align-items: stretch; width: 100%; }
    #veloCarta button {
    ...
    /* ---------- I SEI GESTI ----------
       Solo in rete: contro un avversario vero c'e' qualcuno dall'altra
       parte a cui dire qualcosa, contro il bot non c'e' nessuno ad
       ascoltare. Sei faccine fisse, non testo libero — un tavolo con un
       campo di testo aperto fra sconosciuti e' un problema che non
       vogliamo avere, e sei bastano a dire quasi tutto quello che serve
       durante una partita. */
    #oppProfilo.gesti-disponibili { cursor: pointer; }
    .gesti-scelta {
      display: none; position: fixed; z-index: 860; gap: 4px;
      padding: 6px; border-radius: 10px; background: rgba(20,14,8,0.96);
      border: 1px solid #9a6f21; box-shadow: 0 8px 24px rgba(0,0,0,0.6);
    }
    .gesti-scelta.aperto { display: flex; }
    .gesti-scelta button {
      background: rgba(255,255,255,0.06); border: 1px solid transparent; border-radius: 8px;
      font-size: 20px; padding: 4px 7px; cursor: pointer; line-height: 1;
    }
    .gesti-scelta button:hover { border-color: #e8c46a; background: rgba(232,196,106,0.14); }
    .gesti-scelta button:disabled { opacity: 0.35; cursor: not-allowed; }

    /* la faccina che sale dal nome di chi l'ha mandata, sia per chi la
       manda (conferma "e' partita") sia per chi la riceve */
    .gesto-volante {
      position: fixed; z-index: 850; font-size: 34px; pointer-events: none;
      filter: drop-shadow(0 2px 5px rgba(0,0,0,0.6));
      animation: gestoVolanteSu 2.2s ease-out forwards;
    }
    @keyframes gestoVolanteSu {
      0%   { opacity: 0; transform: translate(-50%, -60%) scale(0.5); }
      15%  { opacity: 1; transform: translate(-50%, -112%) scale(1.2); }
      26%  { transform: translate(-50%, -120%) scale(1); }
      78%  { opacity: 1; transform: translate(-50%, -140%) scale(1); }
      100% { opacity: 0; transform: translate(-50%, -190%) scale(0.85); }
    }
    @media (prefers-reduced-motion: reduce) {
      .gesto-volante { animation: none; opacity: 0.9; transform: translate(-50%, -120%); }
    }
'''
...
<div class="top-shelf">
    ...
    <div class="opp-groups-stack">
        <div class="opp-center-group" id="oppSeatGroup1">
            <div class="opp-profile" id="oppProfilo">
            ...
            <!-- I sei gesti: solo in rete (contro un avversario vero c'è
                 qualcuno dall'altra parte a cui dire qualcosa; contro il
                 bot non c'è nessuno ad ascoltare). Un tocco sul nome li
                 apre — vedi lo script piu' in basso, "I SEI GESTI". -->
            <div class="gesti-scelta" id="gestiScelta">
                <button type="button" data-simbolo="👋" title="Ciao">👋</button>
                <button type="button" data-simbolo="👍" title="Ben giocato">👍</button>
                <button type="button" data-simbolo="😂" title="Ridere">😂</button>
                <button type="button" data-simbolo="😮" title="Sorpreso">😮</button>
                <button type="button" data-simbolo="🤔" title="Pensare">🤔</button>
                <button type="button" data-simbolo="😢" title="Peccato">😢</button>
            </div>
...
const IDS_PERSONAGGI = {
  ...
  // ERA ★4 (110-113), UNA RARITA' INTERA SOPRA la dotazione di partenza
  // (★3): un giocatore nuovo, con la sua prima squadra, ci si scontrava
  // per la sua prima partita vera e si ritrovava con colpi subiti circa
  // il 28% piu' forti dei propri (vita e attacco di un ★4 sono piu' alti
  // su tutta la linea) — "il bot mi distrugge, i miei colpi fanno
  // pochino", segnalato da chi l'ha giocata per davvero. Ora e' la
  // stessa squadra della dotazione iniziale (EROI_DI_PARTENZA in
  // engine/dotazione.js): la prima partita e' alla pari.
  avv: ['personaggio_106', 'personaggio_107', 'personaggio_108', 'personaggio_105']
};
const IDS_MAGICHE = {
...
function cartaHtml(c, selezionabile) {
  ...
  // IL JOLLY.
  // Ha una sua illustrazione vera (immagini/decorazioni/jolly.webp, un
  // giullare classico da mazzo francese, con "JOLLY" gia' scritto
  // dentro l'immagine): niente valore ne' seme da disegnare sopra,
  // niente div di testo a parte — la carta intera e' quell'immagine,
  // dentro il bordo dorato che la distingue dal resto del mazzo (vedi
  // il commento su .card.jolly, piu' sopra).
  ...
  return '<div class="card ' + cls + (c.isJolly ? ' jolly' : '') + ' disegnata st-' + stile +
         ...
         '" data-cid="' + c.id + '"' + (selezionabile ? ' onclick="ui.tocca(\'' + c.id + '\')"' : '') + '>' +
           (c.isJolly ? '' : corpoCarta(valore, seme, stile)) +
         '</div>';
}
...
function disegnaTutto() {
  ...
  // ...e subito dopo si stringono, se sono troppi per la fascia
  stringiGiochi($('myMelds'));
  stringiGiochi($('oppMelds'));
  ...
  // intestazioni — un lampo d'oro SOLO quando il turno cambia per
  // davvero, non a ogni ridisegno (disegna() gira spesso, per motivi che
  // non c'entrano col turno: un lampo a ogni chiamata sarebbe rumore)
  {
    const testoTurno = S.status !== 'in_progress' ? 'Fine' : (S.currentPlayerIndex === 0 ? 'Tocca a te' : 'Avversario');
    const indicatore = $('turnIndicator');
    if (indicatore.textContent !== testoTurno) {
      indicatore.textContent = testoTurno;
      indicatore.classList.remove('cambiato');
      void indicatore.offsetWidth; // fa ripartire l'animazione da capo
...
// ------------------------------------------------------------
// I GIOCHI CALATI SI STRINGONO INVECE DI SPARIRE
//
// Ogni gioco calato e' una colonna larga quanto una carta, e le colonne
// stanno una accanto all'altra dentro meta' campo. Finche' sono poche ci
// stanno; poi finiscono lo spazio. La fascia ha overflow-x:auto, quindi
// tecnicamente non si perdeva niente — ma in pratica si': su una partita
// vera, segnalato da chi ci ha giocato, "quando lo spazio e' quasi finito
// va oltre e le nuove scale non si vedono". Una barra di scorrimento
// orizzontale dentro una striscia alta poche decine di pixel, su un
// tavolo dove tutto il resto sta fermo, nessuno la cerca: si cala un
// gioco, sparisce, e sembra che la calata non sia riuscita.
//
// La colonna verticale, dentro se stessa, questo problema lo aveva gia'
// risolto da tempo: le carte di un gioco lungo si sovrappongono lasciando
// vedere solo una striscia in cima (il PASSO qui sotto in colonnaMeld, che
// si assottiglia finche' la colonna rientra in altezza). Qui si fa la
// stessa identica cosa, ma in orizzontale: quando la fila non ci sta, ogni
// colonna scivola sotto la successiva e ne lascia scoperto il bordo
// sinistro, come un mazzo di carte aperto a ventaglio in mano.
//
// Il limite del 62% e' il punto oltre il quale una colonna smetterebbe di
// essere riconoscibile: di ogni gioco resta scoperto almeno il 38% della
// carta, cioe' l'angolo dove sono stampati valore e seme. Se nemmeno cosi'
// ci stanno (servirebbero piu' di una dozzina di giochi calati per lato,
// caso che nel burraco non capita), lo scorrimento orizzontale e' ancora
// li' sotto a fare da rete.
// ------------------------------------------------------------
function stringiGiochi(lato) {
  if (!lato) return;
  const colonne = Array.prototype.slice.call(lato.querySelectorAll('.card-column'));
  // si riparte sempre da zero: la fascia viene ridisegnata a ogni mossa e
  // a ogni ridimensionamento della finestra, e un margine avanzato dal
  // giro prima falserebbe la misura di questo
  colonne.forEach((c) => { c.style.marginRight = ''; });
  if (colonne.length < 2) return;

  const stile = getComputedStyle(lato);
  const utile = lato.clientWidth -
                (parseFloat(stile.paddingLeft) || 0) -
                (parseFloat(stile.paddingRight) || 0);
  if (utile <= 0) return;

  // QUI NON SI PUO' USARE getBoundingClientRect, E VALE LA PENA DIRE
  // PERCHE': su un telefono tenuto in verticale il tavolo si gira in
  // blocco, con una rotazione di 90 gradi applicata a tutta la pagina
  // (provaOrientamento, in cima allo script). Il rettangolo sullo schermo
  // di un elemento ruotato torna con larghezza e altezza scambiate — una
  // fascia larga 401 e alta 66 si misura larga 66. Il primo tentativo
  // sottraeva proprio quei numeri e concludeva ogni volta che ci stava
  // tutto: la funzione non si accendeva mai, oppure si accendeva al
  // massimo con soli tre giochi calati. Verificato dal vivo, non dedotto.
  //
  // Le misure del foglio di stile invece la rotazione non la sentono: si
  // sommano quelle, larghezza piu' margini di ogni pezzo, e cosi' dentro
  // il conto ci finisce da solo anche il riquadro delle Trappole, che sta
  // in testa alla fila e un margine destro ce l'ha. Si legge dallo stile
  // calcolato e non da offsetWidth perche' quello arrotonda all'intero, e
  // con una dozzina di pezzi gli arrotondamenti si sommano fino a far
  // sbordare la fila di qualche pixel proprio quando doveva rientrare.
  function largo(pezzo) {
    const suo = getComputedStyle(pezzo);
    return (parseFloat(suo.width) || pezzo.offsetWidth || 0) +
           (parseFloat(suo.paddingLeft) || 0) + (parseFloat(suo.paddingRight) || 0) +
           (parseFloat(suo.borderLeftWidth) || 0) + (parseFloat(suo.borderRightWidth) || 0);
  }
  let servono = 0;
  for (let i = 0; i < lato.children.length; i++) {
    const pezzo = lato.children[i];
    const suo = getComputedStyle(pezzo);
    servono += largo(pezzo) +
               (parseFloat(suo.marginLeft) || 0) + (parseFloat(suo.marginRight) || 0);
  }
  const eccesso = servono - utile;
  if (eccesso <= 1) return;   // ci stanno: non si tocca niente

  const larghezza = largo(colonne[0]) || 1;
  const giunzioni = colonne.length - 1;
  const passo = Math.min(eccesso / giunzioni, larghezza * 0.62);
  // l'ultima colonna non si accavalla su niente: e' quella appena calata,
  // ed e' giusto che sia l'unica a vedersi per intero
  for (let i = 0; i < giunzioni; i++) {
    colonne[i].style.marginRight = (-passo).toFixed(2) + 'px';
  }
}
...
function aggiornaMonteTempo() {
  ...
  if (!mio || !suo) return;
  const secMio = monteTempo(0), secSuo = monteTempo(1);
  mio.textContent = mmss(secMio);
  suo.textContent = mmss(secSuo);
  // sotto i 30 secondi si accende, chiunque sia di turno: e' un pericolo
  // che sta li' anche mentre l'orologio dell'altro corre, non solo nel
  // preciso istante in cui scatta
  mio.classList.toggle('agli-sgoccioli', S.status === 'in_progress' && secMio > 0 && secMio <= 30);
  suo.classList.toggle('agli-sgoccioli', S.status === 'in_progress' && secSuo > 0 && secSuo <= 30);
}
...
// Coriandoli della vittoria: stessa tecnica di spacchetta.html (una
// carta rara che si apre) — un div per coriandolo, cade da solo, si
// toglie da solo. Qui non serve un CSS diverso, quello ce l'ha già.
...
function apriSchermataFine() {
  ...
  if (S.winner === 0) {
    ...
    lampo.className = 'lampoSconfitta';
    document.body.appendChild(lampo);
    setTimeout(() => lampo.remove(), 1200);
  }
...
//
// RIPARTE DA SOLA SE L'ACCOUNT NON C'E' PIU', come il tutorial gestionale
// (client/tutorial-gestionale.js, stessa logica commentata li' per
// esteso): il flag "completato" vive nel browser, l'account vive sul
// server, e se il magazzino del server perde i dati (deploy senza disco
// persistente, vedi DOVE_SALVO in server/server.js) i due si
// disallineano.
// NIENTE CHIAMATA DI RETE: si confronta il gettone con cui il tutorial
// e' stato completato l'ultima volta (salvato in tutorialFinisci(), piu'
// in basso) con quello di adesso. Se sono diversi, il server ha dovuto
// darne uno nuovo da allora — quello vecchio e' sparito, e con lui le
// carte. Si controlla PRIMA se c'e' gia' un mazzo salvato valido
// (mazzoScelto()): chi ne ha uno non ha bisogno di essere ricontrollato.
// ------------------------------------------------------------
let MODALITA_TUTORIAL = false;   // deciso da decidiModalitaTutorial(), dentro avvia()

async function decidiModalitaTutorial() {
  ...
  try {
    ...
    if (localStorage.getItem('bb_tutorial_tavolo_completato') !== 'si') return true;
    if (mazzoScelto()) return false;
    const gettoneOra = localStorage.getItem('bb_gettone');
    const gettoneAllora = localStorage.getItem('bb_tutorial_tavolo_gettone');
    // Nessun riferimento salvato (completato prima di questo controllo,
    // o saltato prima che anche Salta lo salvasse): non c'e' modo di
    // dire se e' cambiato qualcosa. Si registra ORA, cosi' almeno il
    // PROSSIMO cambio di gettone verra' notato.
    if (!gettoneAllora) {
      if (gettoneOra) try { localStorage.setItem('bb_tutorial_tavolo_gettone', gettoneOra); } catch (e) {}
      return false;
    }
    return !!(gettoneOra && gettoneOra !== gettoneAllora);
...
//
// SI RICORDA L'ULTIMA GIA' RACCONTATA (per `quando`, non per contenuto:
// due mosse diverse potrebbero avere campi identici). Serve da quando
// esistono i gesti (vedi mandaGesto/mostraGestoArrivato, piu' sotto):
// mandare una faccina fa scattare cambiata(stanza) sul server, la
// versione del tavolo sale, e senza questo controllo l'ascolta() qui
// sotto ripeteva ogni volta l'ultimo VERO esito — un colpo subito ore
// prima tornava a far tremare la carta e a far volare il numero, solo
// perche' qualcuno aveva mandato un pollice in su.
let ultimoEsitoRaccontato = null;
function raccontaLaMossaDellAltro(esito) {
  if (!esito || esito.giocatore === RETE.io) return;
  if (esito.quando && esito.quando === ultimoEsitoRaccontato) return;
  if (esito.quando) ultimoEsitoRaccontato = esito.quando;
...
const ui = {
  ...
  attivaAbilita(seme) {
    ...
    const t = testo(eroe.cardId);
    // ERA "eroe._ability.parametro || 30" — funzionava solo per la
    // forma vecchia (un solo "effect" in cima alla carta). Le sei carte
    // "a scelta" (Papa Figo, Boto Felipe, Onca-Pintada, Mapinguari,
    // Caipora, Boitata) hanno tutte la forma nuova, "effetti: [...]":
    // eroe._ability.parametro non esiste su nessuna di loro, quindi
    // questa riga mostrava SEMPRE 30%, il numero giusto solo per
    // coincidenza su Papa Figo — sulle altre cinque il danno stimato nel
    // messaggio era sbagliato (il colpo vero restava giusto: lo calcola
    // il motore, non questa riga, che serve solo al testo che si legge
    // PRIMA di toccare il bersaglio). Ora si cerca il vero "danno_da_attacco"
    // dentro gli effetti della carta, qualunque sia la forma.
    const effettiEroe = Array.isArray(eroe._ability && eroe._ability.effetti)
      ? eroe._ability.effetti
      : (eroe._ability && eroe._ability.effect ? [eroe._ability] : []);
    const effettoColpo = effettiEroe.find((e) => e.effect === 'danno_da_attacco');
    const pct = (effettoColpo && Number(effettoColpo.parametro)) || 30;
...
const TUTORIAL_PASSI = [
  ...
  { passo: 22, titolo: 'Le Carte Magiche',
    ...
    // dopo 8 secondi allungava il pannello proprio mentre bisognava
    // leggere il bottone USA sotto di lui — richiesto esplicitamente:
    // "puoi eliminare l'ultima riga... così puoi ridurre la finestra".
    // Il tocco apre #veloCarta, la scheda a tutto schermo con la carta
    // grande e il bottone USA di fianco: quel bottone sta verticalmente
    // in mezzo allo schermo, proprio dove il pannello (fisso in alto a
    // destra) di solito finisce — lo copriva. Segnalato: "quando parli
    // delle carte magiche la finestra copre la scritta usala". Su
    // questo passo e sul prossimo (stessa scheda, la Trappola) il
    // pannello sale piu' vicino al bordo superiore, vedi pannelloAlto
    // in tutorialMostraPasso().
    pannelloAlto: true },
...
function tutorialCostruisciPannello() {
  ...
  stile.textContent = [
    // Era largo 320px, fisso in alto a destra: sul tavolo (bloccato in
    ...
    // non far toccare il testo al bordo dorato). Il pannello intero si
    // sposta un po' piu' su (+62px → +50px dal bordo superiore) — poi
    // avanti e indietro di un paio di tentativi ("troppo sotto",
    // "troppo sopra") fino a un punto di mezzo: +54px.
...
function tutorialMostraPasso() {
  ...
  // il bottone USA di fianco, verticalmente in mezzo allo schermo) lo
  // fanno salire, per non finirci sopra.
  tutorialPannelloEl.style.top = def.pannelloAlto ? 'max(10px, env(safe-area-inset-top))' : '';
...
function tutorialFinisci() {
  ...
  try {
    ...
    // con quale gettone si e' finito: vedi decidiModalitaTutorial piu' in alto
...
(async function avvia() {
  ...
  // LA PRIMA MANO GUIDATA (vedi decidiModalitaTutorial piu' sopra): prende
  // il posto della partita normale, non si aggiunge a lei.
  MODALITA_TUTORIAL = await decidiModalitaTutorial();
  ...
  S = createMatch({
    ...
    // gli stessi dieci secondi anche contro il bot: e' il momento in cui
...
out.append('\n<script>\n(function(){\n"use strict";\n'
  ...
  "  } catch (e) {}\n"
  "}\n"
  # SEGNALATO CON UNA FOTO: "mi esce la scritta metti orizzontalmente
  # ma ci sto già". Sulla carta non dovrebbe succedere — la regola qui
  # sopra (#ruotaAvviso { display: none !important }, nello stesso
  # @media hover:none+orientation:portrait della regola che lo
  # accenderebbe) lo tiene spento ogni volta che il trucco
  # dell'orizzontale forzato è attivo, e quando il telefono è DAVVERO
  # in orizzontale quella regola non parte nemmeno: in teoria il
  # riquadro non ha mai un modo di restare acceso. In pratica, su un
  # telefono vero, e' successo lo stesso — segno che su quel browser
  # (probabilmente una vecchia WebView Android: sono note per
  # sbagliare la combinazione hover+orientation) la coppia di media
  # query non si e' aggiornata insieme, o non si e' aggiornata affatto
  # dopo la rotazione. Anziché fidarsi solo del CSS, qui si rifà la
  # stessa domanda in JavaScript — window.innerWidth/innerHeight sono
  # sempre esatti, si ricalcolano ad ogni evento — e si forza lo stato
  # giusto con setProperty(..., 'important'): un !important scritto da
  # JS vince su qualunque !important scritto nel foglio di stile, quindi
  # anche se la regola CSS sbagliasse di nuovo, questa la scavalca.
  "function sistemaAvvisoRotazione() {\n"
  "  var el = document.getElementById('ruotaAvviso');\n"
  "  if (!el) return;\n"
  "  var davveroVerticale = window.innerHeight > window.innerWidth;\n"
  "  el.style.setProperty('display', davveroVerticale ? '' : 'none', 'important');\n"
  "}\n"
  "sistemaAvvisoRotazione();\n"
  "window.addEventListener('resize', sistemaAvvisoRotazione);\n"
  "window.addEventListener('orientationchange', sistemaAvvisoRotazione);\n"
