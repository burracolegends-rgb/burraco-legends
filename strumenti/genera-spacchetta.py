...
# L'involucro era interamente disegnato in SVG (rettangoli, testo, i
# quattro semi in un cerchio) — segnalato come "bruttissimo" da chi lo
# guardava sul serio. Sostituito da un'illustrazione vera (stessa
# palette viola/oro), generata apposta e ripulita a mano di uno
# sparkle finito nell'angolo per conto suo. L'immagine e' gia' tagliata
# alla stessa proporzione 200:280 del vecchio viewBox, quindi tutta la
# geometria dello strappo (DENTI, TAGLIO_ALTO/BASSO, calcolati sopra)
# resta valida senza toccarla.
# La dentellatura e l'orlo restano disegnati: sono linee sottili, non
# hanno senso "fotografate" — un overlay SVG trasparente sopra
# l'immagine, ritagliato insieme a lei dallo stesso clip-path.
INVOLUCRO = (
    '<img class="arte-pacchetto" src="immagini/decorazioni/pacchetto-carte.webp" alt="" draggable="false">' +
    '<svg class="linee-strappo" viewBox="0 0 200 280" xmlns="http://www.w3.org/2000/svg">' +
...
<style>
  ...
  * { box-sizing: border-box; }
  /* ERA overflow:hidden SECCO — bastava per un pacchetto piccolo (il
     riepilogo ci sta in una schermata), ma un pacchetto da 25 o 50
     carte fa una griglia più alta dello schermo, e con body centrato
     in verticale (justify-content:center qui sotto) le prime file
     restavano sopra il bordo, tagliate fuori per sempre — segnalato
     dal vivo: "vedo solo le ultime carte".
     DUE TENTATIVI PRIMA DI QUESTO, ENTRAMBI INSUFFICIENTI (segnalati dal
     vivo tutti e due): 1) overflow-y:auto su "html, body" insieme — con
     ENTRAMBI alti esattamente 100%, l'html (che è l'elemento che scrolla
     davvero, vedi document.scrollingElement) non trabocca mai — è il
     BODY, dentro di lui, a traboccare, in un contenitore di scroll
     annidato che il tocco/rotellina raggiungono a fatica: si scrollava
     pochissimo. 2) altezza normale ma "height:100%" lasciato sia su
     html sia su body: stesso problema, l'html resta bloccato a 100% e
     non puo' mai crescere per contenere un body piu' alto di lui.
     LA CURA VERA: l'html NON deve avere un'altezza fissa — deve poter
     crescere quanto serve, cosi' e' la PAGINA INTERA a scrollare (il
     meccanismo normale di qualunque sito), non un riquadro annidato.
     Al body basta un'altezza MINIMA (min-height, non height) di tutto lo
     schermo, cosi' il fondale riempie comunque lo schermo quando il
     contenuto e' corto. */
  html { margin: 0; }
  body {
    ...
    background:
      radial-gradient(ellipse at 50% 42%, rgba(36,26,53,0.72) 0%, rgba(18,13,28,0.85) 55%, rgba(10,7,15,0.92) 100%),
      url('immagini/decorazioni/sfondo-tavolo.webp') center / cover no-repeat;
    display: flex; flex-direction: column; align-items: center;
    /* LO SCROLL DA SOLO NON BASTAVA — segnalato di nuovo dal vivo: si
       scrollava, ma pochissimo, e delle 50 carte se ne vedevano solo
       10-12. Motivo: "justify-content: center" da solo centra il
       contenuto anche quando trabocca, e mezzo trabocco finisce SOPRA
       il bordo, nell'area che lo scroll non raggiunge (si può scrollare
       solo verso il basso rispetto al centro). "safe center" dice al
       browser di centrare SOLO se il contenuto ci sta; se trabocca, si
       allinea all'inizio (in alto) — tutto il contenuto diventa
       raggiungibile scrollando. La riga sopra (solo "center") resta come
       ripiego per i browser che non conoscono "safe": quelli continuano
       a centrare sempre, meglio che restare senza nessun allineamento. */
  ...
  .pacchetto .arte-pacchetto { width: 100%; display: block; border-radius: 4%; user-select: none; }
  .pacchetto .linee-strappo { position: absolute; inset: 0; width: 100%; height: 100%; }
  @keyframes ondeggia {
  ...
  /* Erano due pezzi che "esplodevano" in direzioni quasi opposte, ognuno
     ruotando E traslando E (il corpo) rimpicciolendo insieme: troppe
     cose diverse in una volta, e con un'immagine fotorealistica sopra
     (prima era un disegno piatto) si vedeva ancora di piu' quanto fosse
     posticcio — segnalato esplicitamente: "anche l'animazione dello
     strappo è pessima". Rifatta sul gesto vero: si strappa una
     linguetta, non si fa scoppiare un pacchetto. transform-origin resta
     al 18% (la linea dello strappo, vedi .meta) — e' proprio la
     cerniera su cui la linguetta ruota all'indietro, come un coperchio.
     Il corpo non ruota quasi per niente e non rimpicciolisce: scivola
     dritto verso il basso, come ci si aspetta scompaia sotto al tavolo. */
  @keyframes linguettaVia {
    0%   { transform: translate(0,0) rotate(0deg); opacity: 1; }
    20%  { transform: translate(-0.5%, -2%) rotate(-6deg); opacity: 1; }
    50%  { transform: translate(1%, -10%) rotate(-32deg); opacity: 1; }
    100% { transform: translate(10%, -46%) rotate(-96deg); opacity: 0; }
  }
  @keyframes corpoGiu {
    0%   { transform: translate(0,0) rotate(0deg); opacity: 1; }
    35%  { transform: translate(0, 3%) rotate(0.5deg); opacity: 1; }
    100% { transform: translate(0, 52%) rotate(2deg); opacity: 0; }
  }
  ...
  .torna { position: fixed; top: 2vh; left: 2vh; z-index: 8; font-size: 1.275rem; color: var(--tenue); text-decoration: none; display: inline-flex; align-items: center; gap: 10px; }
